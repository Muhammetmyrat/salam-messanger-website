<template>
  <BlockBox>
    <template #header>
      <BlockHeader :title="getLocalizedTitle(block)" :link="`/music/block/${block?.id}`" />
    </template>
    <SSwiper :space-between="32">
      <swiper-slide v-for="track in block?.data" :key="track.id">
        <TrackElement class="track-slide" :track="track" />
      </swiper-slide>
    </SSwiper>
  </BlockBox>
</template>
